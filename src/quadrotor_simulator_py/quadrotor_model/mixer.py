...
class QuadMixer:
    ...
    def construct_mixer(self):
        ...
        self.mixer = np.ones((4,4))
        rsa = self.length*np.sin(self.ms_angle)
        rca = self.length*np.cos(self.ms_angle)
        ms = self.mscale
        self.mixer[1,:] = [-rsa, rsa, rsa, -rsa]
        self.mixer[2,:] = [-rca, rca, -rca, rca]
        self.mixer[3,:] = [-ms, -ms, ms, ms]
        return self.mixer
